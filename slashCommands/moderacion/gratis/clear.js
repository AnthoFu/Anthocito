    ...
    async execute(client, interaction) {
        ...
        if (usuario) {
            ...
            await canal.bulkDelete(filtered, true).then((messages) => {
                interaction.reply({
                    content: `Se han borrado ${messages.size} mensajes del usuario ${usuario}, ¿Lo odias acaso? :c`
                });
            });
        } else {
            await canal.bulkDelete(cantidad, true).then((messages) => {
                interaction.reply({
                    content: `Se han borrado ${messages.size} mensajes del canal ${canal} Awebo que sí`
                });
            });
        }
    }
